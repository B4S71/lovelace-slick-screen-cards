console.info("%c WindCompassCard Loaded: V_0.1 ", "color: white; background: #607d8b; font-weight: bold;");
...
class WindCompassCard extends HTMLElement {
  set hass(hass) {
    if (!this.content) {
      ...
      this._historyData = new Array(this._bucketCount).fill().map(() => ({ duration: 0, totalSpeed: 0 }));
    ...
    this._warnMultiplier = (this.config.warn_multiplier !== undefined) ? parseFloat(this.config.warn_multiplier) : 0.9;
    
    // --- SMART BUCKET CALCULATION ---
    let reqBucketSize = (this.config.bucket_size !== undefined) ? parseInt(this.config.bucket_size) : 5;
    ...
    if (this._bucketSize !== finalBucketSize) {
        console.info(`WindCompass: Bucket Size adjusted from config ${this.config.bucket_size || 5} to ${finalBucketSize} to fit 360°.`);
    ...
    if (entityLimitRaffstore && hass.states[entityLimitRaffstore]) {
        this._limitRaffstore = parseFloat(hass.states[entityLimitRaffstore].state);
    }
  ...
  async _fetchHistory(hass, dirEntity, speedEntity) {
    const endTime = new Date();
    const startTime = new Date(endTime.getTime() - 24 * 60 * 60 * 1000); 
    try {
        const entities = [dirEntity, speedEntity].join(',');
        const history = await hass.callApi('GET', `history/period/${startTime.toISOString()}?filter_entity_id=${entities}&end_time=${endTime.toISOString()}&minimal_response`);
        
        if (history && history.length === 2) {
            let dirHist = history.find(arr => arr.length > 0 && arr[0].entity_id === dirEntity);
            let speedHist = history.find(arr => arr.length > 0 && arr[0].entity_id === speedEntity);
  ...
  _processHistoryData(dirData, speedData) {
    const buckets = new Array(this._bucketCount).fill().map(() => ({ duration: 0, totalSpeed: 0 }));
    let speedIdx = 0;
    ...
    for (let i = 0; i < dirData.length - 1; i++) {
        ...
        while(speedIdx < speedData.length - 1 && new Date(speedData[speedIdx+1].last_changed).getTime() <= startTs) {
            speedIdx++;
        }
  ...
  _updateVisuals() {
    if (this._avgDeg === undefined || !this._pxPerDeg) return;

    const tape = this.content.querySelector('#tape');
    if(tape) {
        const offset = this._avgDeg * this._pxPerDeg;
        tape.style.transform = `translateX(-${offset}px)`;
    }

    const marker = this.content.querySelector('#marker');
    ...
    const speedBar = this.content.querySelector('#speed-bar');
    const gustBar = this.content.querySelector('#gust-bar');
    const speedText = this.content.querySelector('#speed-text');
    const limitRaff = this.content.querySelector('#limit-raffstore');
    const limitRollo = this.content.querySelector('#limit-rollo');
    ...
    if (this._currentSpeed !== undefined) {
        ...
        if(speedText) {
            ...
            if (this._currentGust > 0) html += `<span class="faded">(Max ${this._currentGust})</span>`;
  ...
  _updateDimensions() {
    const container = this.content.querySelector('#container');
    if (!container) return;
        ...
        if (bucketIndex >= 0 && bucketIndex < this._bucketCount) {
            ...
            if (data && data.duration > 0) {
                ...
                if (isCritical) {
                    ...
                    fill.style.opacity = 0.1 + (freqRatio * 0.9);
                }
  ...
  setConfig(config) {
    if (!config.direction_entity) throw new Error('direction_entity fehlt');
